...
def test_choose_package_manager_after_configuration(project: Project):
    assert isinstance(project.having_configuration(), LanguageChoice)


def test_choose_language_after_configuration(project: Project):
    assert isinstance(project.having_configuration().using_python(), PythonPackageManagerChoice)


def test_choose_test_manager_after_package_manager(project: Project):
    assert isinstance(project
                      .having_configuration().using_python()
                      .then_test_project()
                      , PythonTestManagerChoice)

def test_sample_choice_after_test_manager(project: Project):
    assert isinstance(project
                      .having_configuration().using_python()
...
def test_deploy_choice_after_sample_choice(project: Project):
    assert isinstance(project
                      .having_configuration().using_python()
